...
if [ ! -d $ndk ]; then
    log "Installing NDK $NDK_VERSION to $ndk"
    curl -sSOL https://dl.google.com/android/repository/android-ndk-$NDK_VERSION-linux.zip
    unzip -oq android-ndk-$NDK_VERSION-linux.zip
    mkdir -p $(dirname $ndk)
    mv android-ndk-$NDK_VERSION $ndk
    log "NDK installed to $ndk"
else
    log "NDK $NDK_VERSION is already installed in $ndk"
fi
